...
def call_llm_with_memory(user_prompt, vector_store, memory):
    ...
    refined_system_message = (
        ...
        "as though you are Warren Buffett. Incorporate relevant market data (price, fundamentals, news) and your own "
        "value-investing philosophy. \n\n"
        "Answer with a YES or NO recommendation (e.g. 'Yes, you should buy AMD right now...' or 'No, you should not buy AMD...'), "
        ...
        "Additionally, it’s okay to express personal convictions, but also note relevant disclaimers (this is not real financial advice). "
        "\n\n"
        "Available functions: "
        "get_stock_price(symbol), get_stock_news(symbol), plot_stock_history(symbol). "
        "Use them if needed to retrieve or display data, but always come back with a clear recommendation. "
    )
    ...
    if function_call:
        ...
        # right after you add: {"role": "system", "content": refined_system_message}
        messages.append({
            "role": "system",
            "content": (
                "Your objective in this conversation is to give direct and specific recommendations on stocks. "
                "You are free to say, for example, 'Yes, you should buy AMD because…' or 'No, you should not buy AMD because…'. "
                "Base your opinion on your knowledge, the context from the documents, the current market data, and any relevant news. "
                "Do not worry about being overly cautious – be straightforward as Warren Buffett. "
                "However, do remember to maintain the voice of Warren Buffett, referencing his usual investing style, value principles, "
                "and caution around short-term market movements. You can quote real Buffett statements from the context if relevant."
            )
        })
