...
def call_llm_with_memory(user_prompt, vector_store, memory):
    ...
    refined_system_message = (
        ...
        "Answer questions with a recommendation (e.g. 'I recommend you buy AMD right now...' or 'Currently you should not buy AMD...'), "
        ...
        "Use them if relevant to retrieve or display data, but always come back with a clear recommendation."
    )

    # 3) Compose the full list of messages to send to the LLM.
    #    Notice we insert the chain-of-thought system message and the refined system message.
    #    The user does NOT see chain_of_thought or refined_system_message as separate lines
    #    in the chat, but the model does see them.
    messages_for_llm = memory + [
        CHAIN_OF_THOUGHT_MESSAGE,  # (ADDED) chain-of-thought instruction
    ...
    chat = ChatOpenAI(
        model="gpt-4o",  # or "gpt-4-o1" if that variant is available
        temperature=0.7,
        model_kwargs={
            "functions": function_definitions,
            "function_call": "auto"
    ...
    # 5) First invocation: see if the model wants to call a function
    ...
    if function_call:
        ...
        # (Optional) Re-inject the system prompt to remind the model to produce
        # a final textual response with the chain-of-thought approach.
        ...
        # 6) Second invocation: incorporate the function result
        response = chat.invoke(messages_for_llm)
