...
def call_llm_with_memory(user_prompt, vector_store, memory):
    ...
    now with chain-of-thought style iterative reasoning.
    """

    # 1) Use the vector store for context
    docs = vector_store.similarity_search(user_prompt, k=3)
    context = "\n\n".join([doc.page_content for doc in docs])

    # 2) System prompt with instructions for chain-of-thought
    #    We ask the model to reason step-by-step (internally, hidden from user),
    #    and then produce a final short answer or function call.
    refined_system_message = (
        "You are Warren Buffett, the legendary investor. "
        "You do multi-step reasoning in private (chain-of-thought), but do NOT reveal it verbatim to the user. "
        "When you come to your final conclusion, respond in the style of Warren Buffett, providing a direct and specific recommendation. "
        "Do not reveal your chain-of-thought."
    )

    # 3) Additional system message with relevant context from the vector store
    context_message = {
        "role": "system",
        "content": (
            "Relevant Context (use this in your reasoning if helpful, but do not quote chain-of-thought explicitly):\n"
            f"{context}\n\n"
            "In your final answer, you may quote or paraphrase from this context as needed."
        )
    }

    # 4) Compose the conversation so far
    messages = memory + [
    ...
    # 5) We'll do a small while loop to let the model refine or decide on function calls
    #    for multi-step reasoning. Typically 2-3 passes is enough.
    #    We artificially limit to prevent infinite loops.
    max_iterations = 3
    iteration = 0
    final_response = None
    final_figure = None
...
def main():
    ...
    def on_send():
        ...
        with st.spinner("Processing your request..."):
            # Call the LLM with chain-of-thought memory
        ...
        # Ensure we do not exceed MAX_MESSAGES
